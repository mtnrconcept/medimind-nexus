-- Migration: Fix KG Seed Function Timeout
-- Issue: Function times out due to PostgREST client timeout limit
-- Solution: Increase timeout and use SECURITY DEFINER for privileged execution
-- Fixes: Uses correct cde_edges column names (relationship_type not edge_type)
-- Created: 2025-12-16

-- Drop and recreate with higher timeout and SECURITY DEFINER
...
BEGIN
  ...
  JOIN cde_nodes mn ON mn.external_id = ms.medication_id AND mn.node_type = 'medication'
  LIMIT 5000;
