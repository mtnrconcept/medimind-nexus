-- MIGRATION: Complete rebuild of seed_cde_knowledge_graph
-- 2025-12-17 14:35 - Fix atc_code error by using only existing columns
-- This completely replaces the function
...
-- Recreate with ONLY safe columns that definitely exist
...
DECLARE
    ...
    v_result JSONB;
BEGIN
    RAISE NOTICE 'Starting CDE Knowledge Graph seeding (v3 - no atc_code)...';

    -- ============================================
    -- PHASE 1: Insert SUBSTANCES (name only - no atc_code!)
    -- ============================================
    ...
    JOIN cde_nodes src ON LOWER(TRIM(src.name)) = LOWER(TRIM(m.name))
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION public.seed_cde_knowledge_graph() TO authenticated;
GRANT EXECUTE ON FUNCTION public.seed_cde_knowledge_graph() TO service_role;
GRANT EXECUTE ON FUNCTION public.seed_cde_knowledge_graph() TO anon;
