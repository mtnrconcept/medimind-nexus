-- Fix CDE Nodes - Clean up invalid substance names
-- This migration removes entries that contain JSON fragments like "dosage":"..." 
-- and reorganizes data extraction from medications.substance field

-- ============================================
-- STEP 1: Delete invalid cde_nodes entries
-- ============================================

-- Remove nodes that contain JSON patterns or dosage fragments
DELETE FROM cde_edges 
WHERE source_node_id IN (
  SELECT id FROM cde_nodes 
  WHERE name LIKE '%"dosage"%'
     OR name LIKE '%":%'
     OR name LIKE '{%'
     OR name LIKE '%}%'
     OR name LIKE '%null%'
...
-- ============================================
-- STEP 2: Also clean substances table if it exists
-- ============================================

DELETE FROM medication_substances 
...
-- ============================================
-- STEP 3: Update the seed function with better validation
-- ============================================
...
BEGIN
  ...
  filtered AS (
    ...
    WHERE LENGTH(substance_name) > 2
      AND LENGTH(substance_name) < 200
      -- Exclude JSON fragments
      AND substance_name NOT LIKE '%"dosage"%'
      AND substance_name NOT LIKE '%":%'
      AND substance_name NOT LIKE '{%'
      AND substance_name NOT LIKE '%}%'
      AND substance_name NOT LIKE '%null%'
      -- Exclude pure numbers or dosages
      AND substance_name !~ '^[0-9.,\s]+$'
      AND substance_name !~ '^\d+.*mg$'
      AND substance_name !~ '^\d+.*g\s*pro\s*\d+\s*g'
      AND substance_name !~ '^max\.\s*\d+'
      -- Must contain at least one letter
      AND substance_name ~ '[a-zA-Z]{2,}'
  )
  ...
  -- Add substances from drug_interactions (also with validation)
  ...
  WHERE interacting_drug IS NOT NULL 
    ...
    AND LENGTH(TRIM(interacting_drug)) < 200
    AND interacting_drug NOT LIKE '%"dosage"%'
    AND interacting_drug NOT LIKE '%":%'
    AND interacting_drug ~ '[a-zA-Z]{2,}'
  ON CONFLICT (name_normalized) DO NOTHING;
  ...
  -- ============================================
  -- PHASE 3: Insert INDIVIDUAL substances into cde_nodes
  -- ============================================
  ...
  result := jsonb_build_object(
    ...
    'message', 'CDE reset complete with validated substances'
  );
